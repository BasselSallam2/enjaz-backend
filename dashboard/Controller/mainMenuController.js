...
export const overview = async (req, res, next) => {
  try {
    ...
    if (title === "admin" || title === "superadmin") {
      ...
      const orders = await prisma.orders.findMany({where:{isDeleted:false},
      ...
      const orders = await prisma.orders.findMany({
        where: { isDeleted:false , employeeId: employeeId, status: { not: "Cancelled" } },
      });
...
export const urgentOrders = async (req, res, next) => {
  try{
    ...
    const orders = await prisma.orders.findMany({
      ...
      status: { notIn: ["Finished", "In delivery"] },
      }, select:{number:true , type:true , numberofletters:true , user:true , createdAt:true ,status:true ,cost:true}
    });

    const urgentOrders = orders.filter((order) => {
      return order.createdAt.getTime() < Date.now() - 5 * 24 * 60 * 60 * 1000;
    });

    const result = urgentOrders.map((order) => {
      return {
        number: order.number,
        service: order.type,
        ...
        deadline: Math.max(
          0,
          Math.floor((order.createdAt.getTime() + 7 * 24 * 60 * 60 * 1000 - Date.now()) / (1000 * 60 * 60))
        ),
        status: order.status,
        cost: order.cost,
      };
